...
BoW(词袋模型Bag-of-words model): 将一段文本(句子或者文档)，用一个"装着这些词的袋子"来表示，这种表示方法不考虑文法以及词序. **在使用词袋模型时，文档的向量表示直接将各个词的词频向量表示加和.** 缺点:
- 词向量化后，词与词之间是有权重大小关系的，不一定词出现的越多，权重越大;
- 词与词之间是没有顺序关系的.

Word2Vec: 一种考虑词位置关系的模型. 通过大量的语料训练，将每个词映射成一个低维稠密向量，通过余弦计算，可以判断两个词语之间的关系，其底层主要采用基于CBOW和Skip-Gram算法的神经网络模型. 相较于BoW，改进主要为:
- 考虑了词与词之间的顺序，引入了上下文的信息;
- 得到了词更加准确的表示，其表达的信息更为丰富.

BERT的思想其实很大程度上来源于CBOW模型，如果从准确率上说改进的话，BERT利用更深的模型，以及海量的语料，得到的embedding表示，来做下游任务时的准确率是要比Word2Vec高不少的( 实际上，这也离不开模型的“加码”以及数据的“巨大加码”). 再从方法的意义角度来说，BERT的重要意义在于给大量的NLP任务提供了一个泛化能力很强的预训练模型，而仅仅使用Word2Vec产生的词向量表示，不仅能够完成的任务比BERT少了很多，而且很多时候直接利用Word2Vec产生的词向量表示给下游任务提供信息，下游任务的表现不一定会很好，甚至会比较差.