...
- 许多主流的神经语言模型都会先给序列中的Token计算一个**上下文无关**的Token表示$x_k^{LM}$，然后将它传递给L层前向LSTM. 这样的话，前向和后向语言模型分别在每个位置k，每个LSTM层输出一个**上下文相关**的表示$\vec{h}_{k,j}^{LM},  \mathop{h}\limits^{\leftarrow}_{k,j}^{LM}$，其中$j = 1, ..., L$(ELMo中取L=2).